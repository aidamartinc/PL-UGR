...
void Check_ListTernary(attrs expr1, attrs op1, attrs expr2, attrs op2, attrs expr3, attrs *res) {
  if (!isList(expr1) || listToType(expr1.type) != expr2.type || expr3.type != ENTERO) {
    printf("%s[SEMANTIC ERROR @ line %d] Ternary operator ++ @ expects expressions of types [LIST OF x]++[x]@[ENTERO], but got types [%s]++[%s]@[%s]\n",
